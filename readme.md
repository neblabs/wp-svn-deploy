...
Deploys a build dir to a *new* tag with the given version to your plugin's WordPress repo using svn. Now instead of deploying to trunk, this will deploy it to a new tag. This is by design so you can test that the deployed build was uploaded correctly and without corruptions. After you have tested the new tag, you just have to cp the tag to the trunk and then it will be live.
...
- Trunk is not touched. You have to update trunk after the new tag is deployed.
...
```bash
wp-snv-deploy -slug slug -build dir -version n [--batch-size n] [--message message]
```
